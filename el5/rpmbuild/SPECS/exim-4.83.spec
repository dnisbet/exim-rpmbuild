...
%define spf_ver 1.2.9
%define _rc 2
%define _with_dmarc 1
%define _with_prdr 1
%define _with_proxy 1

# To build in mock successfully, gotta give a little wiggle room
...
#%if 0%{?_rc}
#Release: 0.1.RC%{_rc}.%{?dist}
#%define _src_name %{name}-%{version}_RC%{_rc}
#Source: ftp://ftp.exim.org/pub/exim/exim4/test/%{_src_name}.tar.bz2
#Source1: ftp://ftp.exim.org/pub/exim/exim4/test/%{_src_name}.tar.bz2.asc
#%else
Release: 1%{?dist}
%define _src_name %{name}-%{version}
Source: ftp://ftp.exim.org/pub/exim/exim4/%{_src_name}.tar.bz2
Source1: ftp://ftp.exim.org/pub/exim/exim4/%{_src_name}.tar.bz2.asc
#%endif
...
Source51: libspf2-%{spf_ver}.tar.gz
Source52: libsrs_alt-1.0.tar.gz

Patch4: exim-rhl.patch
Patch6: exim-4.80-config.patch
Patch7: exim-4.69-config-atrpms.patch
...
Patch31: exim-4.74-bV-lookups.patch
Patch51: libspf2.patch
...
Requires: libopendmarc opendmarc-tools
%endif
...
sed -i 's@          tls_sni);@          tls_out.sni);@' src/tls-openssl.c

%setup -q -T -D -a 51 -n %{_src_name}
pushd libspf2-%{spf_ver}
%patch51 -p0 -b .spf
...
echo "ac_cv_func___ns_get16=no" >> config.cache
popd

echo "EXPERIMENTAL_SPF=yes" >> Local/Makefile
echo "CFLAGS += -DSPF -I../src -I../../src" >> Local/Makefile
echo "LDFLAGS += -L../src" >> Local/Makefile
#echo "LOOKUP_SPF=2" >> Local/Makefile

%setup -q -T -D -a 52 -n %{_src_name}
echo "EXPERIMENTAL_SRS=yes" >> Local/Makefile
echo "CFLAGS += -I../src/srs/src -I../../src/srs/src -I../../src/srs" >> Local/Makefile
echo "LDFLAGS += -lcompat" >> Local/Makefile

%if 0%{?_with_dmarc}
# Enable DMARC capability
echo "EXPERIMENTAL_DMARC=yes" >> Local/Makefile
echo "LDFLAGS += -lopendmarc" >> Local/Makefile
%endif

%if 0%{?_with_prdr}
# Enable PRDR code
echo "EXPERIMENTAL_PRDR=yes" >> Local/Makefile
%endif

echo "EXPERIMENTAL_CERTNAMES=yes" >> Local/Makefile

%if 0%{?_with_proxy}
# Enable Proxy Protocol support
sed -i "s@^# EXPERIMENTAL_PROXY=yes@EXPERIMENTAL_PROXY=yes@" Local/Makefile
%endif

# Appends to _RCn string
#echo 'EXIM_VARIANT_VERSION="${EXIM_VARIANT_VERSION}+test1"' >> src/version.sh
...
# Build SRS
pushd libsrs_alt-1.0
%configure --with-pic
popd

# Setup libs into appropriate location so linker can find them
ln -sf ../libspf2-%{spf_ver}/src/include src/spf2
pushd libspf2-%{spf_ver}
make
popd
rsync -L libspf2-%{spf_ver}/src/libspf2/.libs/libspf2.so src/
perl -pi -e 's/\b(SQLITE)\b/SPF $1/' scripts/lookups-Makefile
ln -sf ../libsrs_alt-1.0/ src/srs
pushd libsrs_alt-1.0
make CFLAGS=-fPIC
popd
rsync -L libsrs_alt-1.0/{src/libsrs_alt.a,lib/libcompat.a} src/
...
#for i in mysql.so pgsql.so spf.so
...
install -m755 src/libspf2.so $RPM_BUILD_ROOT/%{_libdir}/exim/
mkdir -p $RPM_BUILD_ROOT/%{_sysconfdir}/ld.so.conf.d
echo '%{_libdir}/exim' > $RPM_BUILD_ROOT/%{_sysconfdir}/ld.so.conf.d/exim.conf
for SPF in spfd spfquery spftest; do
  install -m755 libspf2-%{spf_ver}/src/$SPF/$SPF $RPM_BUILD_ROOT/%{_sbindir}/exim-$SPF
done
install -m755 libsrs_alt-1.0/srs $RPM_BUILD_ROOT/%{_sbindir}/exim-srs
install -m755 libsrs_alt-1.0/test $RPM_BUILD_ROOT/%{_sbindir}/exim-srs-test
...
## %{_libdir}/exim/%{version}-%{release}/lookups/spf.so
%{_libdir}/exim/libspf2.so
...
%{_sysconfdir}/ld.so.conf.d/exim.conf
...
%doc libspf2-%{spf_ver}/{INSTALL,LICENSES,README,TODO}
...
%changelog
* Fri Jun 06 2014 Todd Lyons <tlyons@ivenue.com> - 4.83-0.1.RC2.iv
- 4.83 RC2

* Sun May 25 2014 Todd Lyons <tlyons@ivenue.com> - 4.82-2iv
- Patch to better parse From header

* Mon Oct 28 2013 Todd Lyons <tlyons@ivenue.com> - 4.82-1iv
- New release

* Fri Oct 26 2012 Todd Lyons <tlyons@ivenue.com> - 4.80.1-1.3iv
- Debugging libopendmarc patch.

* Fri Oct 26 2012 Todd Lyons <tlyons@ivenue.com> - 4.80.1-1iv
- Security release for CVE-2012-5671

* Mon Jun 04 2012 Todd Lyons <tlyons@ivenue.com> - 4.80-2iv
- Update to 4.80.
- Includes refactor MAIL FROM optional args patch.

* Fri Apr 13 2012 Todd Lyons <tlyons@ivenue.com> - 4.77-2.2iv
- Bugfix: missing break statement

* Wed Mar 28 2012 Todd Lyons <tlyons@ivenue.com> - 4.77-2.1iv
- Bugfix: two missing breaks in arg processing

* Wed Mar 21 2012 Todd Lyons <tlyons@ivenue.com> - 4.77-2iv
- Refactor parsing optional args to MAIL FROM

* Mon May  9 2011 Todd Lyons <tlyons@ivenue.com> - 4.76-1iv
- Update to 4.76.
- CVE-2011-1764: a format string attack in logging DKIM

* Fri Mar 25 2011 Todd Lyons <tlyons@ivenue.com> - 4.75-1iv
- Update to 4.75.

* Fri Feb  4 2011 Todd Lyons <tlyons@ivenue.com> - 4.74-1.1iv
- Show dynamically loaded modules installed, in -bV output.

* Tue Jan 25 2011 Todd Lyons <tlyons@ivenue.com> - 4.74-1iv
- Update to 4.74 new release.

* Thu Jan  6 2011 Todd Lyons <tlyons@ivenue.com> - 4.73-1iv
- Update to 4.73.

* Thu Sep  9 2010 Todd Lyons <tlyons@ivenue.com> - 4.72-1iv
- Rebuild for Ivenue.
